...
public:
    ...
    real_scalar_t log_posterior(real_vector_t const & arg_z) const {
        BOOST_ASSERT(arg_x.size() == m_n + 1); // x and omega
...
private:
    real_scalar_t m_n;
    real_scalar_t m_omega;
    real_scalar_t m_zeta
    real_vector_t m_y;
};

template<typename real_scalar_t>
test_var_est_gaussian_noise(){
    variance_estimation<real_scalar_t> var_est;
}
