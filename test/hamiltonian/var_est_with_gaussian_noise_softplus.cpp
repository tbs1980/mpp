#define BOOST_TEST_MODULE var_est gaussian_noise_soft_plus
...
class variance_estimation_softplus {
public:
    ...
    real_scalar_t log_posterior(real_vector_t const & arg_z) const {
        ...
        real_scalar_t const omega = std::log(1. + std::exp(nu));
        if(omega <= 1e-5 or std::isfinite(omega) == false ){
            return -1e90;
        }
        ...
        return log_y_x + log_x_omega + log_nu_omega;
    ...
    real_vector_t grad_log_posterior(real_vector_t const & arg_z) const {
        ...
        d_omega += 1./(std::exp(omega) - 1.);

        // derivative with respect to nu
        real_scalar_t const d_nu = d_omega*(1. - std::exp(-omega));

        // assign d_x and d_nu to d_z
    ...
    real_matrix_t metric_tensor_log_posterior(
        ...
        // get nu from z and calculate omega
        real_scalar_t const nu = arg_z(m_n);
        real_scalar_t const omega = std::log(1. + std::exp(nu));
        ...
        G(m_n,m_n) = 0.5*static_cast<real_scalar_t>(m_n)*(1. - std::exp(-omega))/omega/omega;
    ...
    real_matrix_array_t deriv_metric_tensor_log_posterior(
        ...
        for(std::size_t i = 0; i < m_n; ++i) {
            d_G[m_n](i,i) = -(1. - std::exp(-omega))/omega/omega;
        }
        d_G[m_n](m_n,m_n) = -static_cast<real_scalar_t>(m_n)
            *(1. - std::exp(-omega))*((1. - std::exp(-omega))/omega + 0.5*std::exp(-omega) )/omega/omega;
...
void test_var_est_gaussian_noise_softplus(std::string const & chn_file_name){
    using namespace boost::numeric::ublas;
    ...
    typedef variance_estimation_softplus<real_scalar_t> var_est_sp_t;
    typedef std::mt19937 rng_t;
    typedef boost::numeric::ublas::vector<real_scalar_t> real_vector_t;
    ...
    log_post_func_t log_posterior
        = std::bind (&var_est_sp_t::log_posterior, &var_est_sp, _1);
    grad_log_post_func_t grad_log_posterior
        = std::bind (&var_est_sp_t::grad_log_posterior, &var_est_sp, _1);
    mtr_tnsr_log_post_func_t metric_tensor_log_posterior = std::bind (
            &var_est_sp_t::metric_tensor_log_posterior,
            &var_est_sp,
            _1
    );
    der_mtr_tnsr_log_post_func_t deriv_metric_tensor_log_posterior = std::bind(
            &var_est_sp_t::deriv_metric_tensor_log_posterior,
            &var_est_sp,
            _1
    );
    ...
    x_0(num_dims - 1) = std::log(std::exp(1.) - 1.);
    rng_t rng;
...
void test_var_est_gaussian_noise_ensemble(std::string const & chn_file_name) {
    ...
    real_scalar_t const zeta(1.);
    std::size_t const seed = 31415;
    var_est_sp_t var_est(n, omega, zeta, seed);

    // create the functors for RMHMC
    using std::placeholders::_1;
    log_post_func_t log_posterior
        = std::bind (&var_est_sp_t::log_posterior, &var_est, _1);
    ...
    for(std::size_t i=0; i < num_walkers; ++i) {
        ...
        start_ensemble(i, num_dims - 1) = std::log(std::exp(1. + 0.1*nrm_dist(rng)) - 1.);
    }
...
BOOST_AUTO_TEST_CASE(variance_estimation_gaussian_noise_softplus) {
    // test_var_est_gaussian_noise_softplus<float>(
    //     std::string("var_est_gauss_noise_softplus_float.chain")
    // );

    test_var_est_gaussian_noise_ensemble<float>(
        std::string("var_est_gauss_noise_oftplus_ensemble_float.chain")
    );
}
